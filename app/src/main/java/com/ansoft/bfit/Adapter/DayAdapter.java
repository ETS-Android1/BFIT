...
public class DayAdapter extends RecyclerView.Adapter<DayAdapter.MyViewHolder> {
    ...
    public void onBindViewHolder(MyViewHolder viewHolder, final int i) {
        ...
        if (data.getProgress() < 100) {
            viewHolder.dayProgress.setVisibility(View.INVISIBLE);
            viewHolder.dayProgress2.setVisibility(View.INVISIBLE);
        } else {
            viewHolder.dayProgress.setVisibility(View.VISIBLE);
            viewHolder.dayProgress2.setVisibility(View.INVISIBLE);
            viewHolder.dayName.setTextColor(ContextCompat.getColor(context, R.color.colorAccent));
        }
        if (i == current) {
            viewHolder.dayProgress.setVisibility(View.INVISIBLE);
            viewHolder.dayProgress2.setVisibility(View.VISIBLE);
            viewHolder.dayName.setTextColor(ContextCompat.getColor(context, android.R.color.white));
        }
    ...
    class MyViewHolder extends RecyclerView.ViewHolder {
        TextView dayName;
        LinearLayout dayProgress, dayProgress2;
        SquareRelativeLayout outerView;

        public MyViewHolder(View itemView) {
            super(itemView);
            dayName = itemView.findViewById(R.id.dayName);
            dayProgress = itemView.findViewById(R.id.dayProgress);
            outerView = itemView.findViewById(R.id.outerView);
            dayProgress2 = itemView.findViewById(R.id.dayProgress2);
        }
